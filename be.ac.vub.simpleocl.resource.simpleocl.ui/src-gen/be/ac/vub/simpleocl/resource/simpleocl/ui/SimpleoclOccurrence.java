...
public class SimpleoclOccurrence {
	...
	public final static String OCCURRENCE_ANNOTATION_ID = "be.ac.vub.simpleocl.resource.simpleocl.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "be.ac.vub.simpleocl.resource.simpleocl.ui.occurences.declaration";
	...
	private be.ac.vub.simpleocl.resource.simpleocl.ui.SimpleoclTokenScanner tokenScanner;
	 ...
	 */
	public SimpleoclOccurrence(be.ac.vub.simpleocl.resource.simpleocl.ISimpleoclTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, be.ac.vub.simpleocl.resource.simpleocl.ui.SimpleoclTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new be.ac.vub.simpleocl.resource.simpleocl.mopp.SimpleoclMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(be.ac.vub.simpleocl.resource.simpleocl.ui.SimpleoclBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, be.ac.vub.simpleocl.resource.simpleocl.ui.SimpleoclPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, be.ac.vub.simpleocl.resource.simpleocl.ui.SimpleoclPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, be.ac.vub.simpleocl.resource.simpleocl.ui.SimpleoclPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == be.ac.vub.simpleocl.resource.simpleocl.ui.SimpleoclPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(be.ac.vub.simpleocl.resource.simpleocl.ui.SimpleoclOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(be.ac.vub.simpleocl.resource.simpleocl.ui.SimpleoclOccurrence.DECLARATION_ANNOTATION_ID);
	}
