...
public class SimpleoclDefaultNameProvider implements be.ac.vub.simpleocl.resource.simpleocl.ISimpleoclNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = be.ac.vub.simpleocl.resource.simpleocl.util.SimpleoclEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
