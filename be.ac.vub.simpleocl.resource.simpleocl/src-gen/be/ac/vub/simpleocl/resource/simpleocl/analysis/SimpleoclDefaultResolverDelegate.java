...
public class SimpleoclDefaultResolverDelegate<ContainerType extends org.eclipse.emf.ecore.EObject, ReferenceType extends org.eclipse.emf.ecore.EObject> {
	...
	private static class ReferenceCache implements be.ac.vub.simpleocl.resource.simpleocl.ISimpleoclReferenceCache, org.eclipse.emf.common.notify.Adapter {
	...
	protected void resolve(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, be.ac.vub.simpleocl.resource.simpleocl.ISimpleoclReferenceResolveResult<ReferenceType> result) {
		try {
			...
			if (!enableScoping) {
				root = be.ac.vub.simpleocl.resource.simpleocl.util.SimpleoclEObjectUtil.findRootContainer(container);
			}
			while (root != null) {
	...
	private java.util.Set<org.eclipse.emf.ecore.EObject> findReferencedExternalObjects(org.eclipse.emf.ecore.EObject object) {
		org.eclipse.emf.ecore.EObject root = be.ac.vub.simpleocl.resource.simpleocl.util.SimpleoclEObjectUtil.findRootContainer(object);
	...
	private boolean tryToResolveIdentifierAsURI(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, be.ac.vub.simpleocl.resource.simpleocl.ISimpleoclReferenceResolveResult<ReferenceType> result) {
		org.eclipse.emf.ecore.EClass type = reference.getEReferenceType();
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					String result = (String) be.ac.vub.simpleocl.resource.simpleocl.util.SimpleoclEObjectUtil.invokeOperation(element, operation);
					names.add(result);
				}
			}
		}
