...
public abstract class AbstractSimpleoclDebuggable {
	 ...
	 */
	private java.util.List<be.ac.vub.simpleocl.resource.simpleocl.util.SimpleoclPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<be.ac.vub.simpleocl.resource.simpleocl.util.SimpleoclPair<String, Integer>>();
	...
	public void sendEvent(be.ac.vub.simpleocl.resource.simpleocl.debug.ESimpleoclDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			be.ac.vub.simpleocl.resource.simpleocl.debug.SimpleoclDebugMessage message = new be.ac.vub.simpleocl.resource.simpleocl.debug.SimpleoclDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				be.ac.vub.simpleocl.resource.simpleocl.util.SimpleoclPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(be.ac.vub.simpleocl.resource.simpleocl.debug.ESimpleoclDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(be.ac.vub.simpleocl.resource.simpleocl.debug.ESimpleoclDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new be.ac.vub.simpleocl.resource.simpleocl.util.SimpleoclPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new be.ac.vub.simpleocl.resource.simpleocl.util.SimpleoclPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(be.ac.vub.simpleocl.resource.simpleocl.debug.ESimpleoclDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(be.ac.vub.simpleocl.resource.simpleocl.debug.ESimpleoclDebugMessageTypes.STARTED, true);
			setSuspend(true);
