 ...
 */
public class SimpleoclDebuggableInterpreter<ResultType, ContextType> extends be.ac.vub.simpleocl.resource.simpleocl.debug.AbstractSimpleoclDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private be.ac.vub.simpleocl.resource.simpleocl.util.AbstractSimpleoclInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private be.ac.vub.simpleocl.resource.simpleocl.ISimpleoclCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public SimpleoclDebuggableInterpreter(be.ac.vub.simpleocl.resource.simpleocl.util.AbstractSimpleoclInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new be.ac.vub.simpleocl.resource.simpleocl.ISimpleoclInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = be.ac.vub.simpleocl.resource.simpleocl.util.SimpleoclStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public be.ac.vub.simpleocl.resource.simpleocl.util.AbstractSimpleoclInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		be.ac.vub.simpleocl.resource.simpleocl.ISimpleoclLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private be.ac.vub.simpleocl.resource.simpleocl.ISimpleoclLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		be.ac.vub.simpleocl.resource.simpleocl.ISimpleoclCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(be.ac.vub.simpleocl.resource.simpleocl.debug.ESimpleoclDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = be.ac.vub.simpleocl.resource.simpleocl.util.SimpleoclEObjectUtil.getDepth(current);
		stopCondition = new be.ac.vub.simpleocl.resource.simpleocl.ISimpleoclCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = be.ac.vub.simpleocl.resource.simpleocl.util.SimpleoclEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = be.ac.vub.simpleocl.resource.simpleocl.util.SimpleoclEObjectUtil.getDepth(current) - 1;
		stopCondition = new be.ac.vub.simpleocl.resource.simpleocl.ISimpleoclCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
