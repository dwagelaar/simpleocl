...
public class SimpleoclContainmentTrace {
	 ...
	 */
	private be.ac.vub.simpleocl.resource.simpleocl.mopp.SimpleoclContainedFeature[] path;
	
	public SimpleoclContainmentTrace(org.eclipse.emf.ecore.EClass startClass, be.ac.vub.simpleocl.resource.simpleocl.mopp.SimpleoclContainedFeature[] path) {
		super();
		// Verify arguments
		if (startClass != null) {
			if (path.length > 0) {
	...
	public be.ac.vub.simpleocl.resource.simpleocl.mopp.SimpleoclContainedFeature[] getPath() {
		return path;
	}
	
	public String toString() {
		return (startClass == null ? "null" : startClass.getName()) + "->" + be.ac.vub.simpleocl.resource.simpleocl.util.SimpleoclStringUtil.explode(path, "->");
	}
