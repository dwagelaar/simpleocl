...
public class SimpleoclTaskItemDetector {
	...
	public java.util.List<be.ac.vub.simpleocl.resource.simpleocl.mopp.SimpleoclTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<be.ac.vub.simpleocl.resource.simpleocl.mopp.SimpleoclTaskItem> foundItems = new java.util.ArrayList<be.ac.vub.simpleocl.resource.simpleocl.mopp.SimpleoclTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new be.ac.vub.simpleocl.resource.simpleocl.mopp.SimpleoclTaskItem(keyword, message, localLine, offset, end));
