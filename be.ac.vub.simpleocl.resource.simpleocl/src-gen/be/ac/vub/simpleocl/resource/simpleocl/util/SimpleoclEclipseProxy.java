...
public class SimpleoclEclipseProxy {
		...
		if (org.eclipse.core.runtime.Platform.isRunning()) {
			...
			for (org.eclipse.core.runtime.IConfigurationElement element : configurationElements) {
				try {
					...
					for (Object key : keys) {
						be.ac.vub.simpleocl.resource.simpleocl.util.SimpleoclMapUtil.putAndMergeKeys(optionsMap, key, options.get(key));
					}
				} catch (org.eclipse.core.runtime.CoreException ce) {
					new be.ac.vub.simpleocl.resource.simpleocl.util.SimpleoclRuntimeUtil().logError("Exception while getting default options.", ce);
				}
			}
		}
	}
	 ...
	 */
	public be.ac.vub.simpleocl.resource.simpleocl.mopp.SimpleoclResource getResource(org.eclipse.core.resources.IFile file) {
	...
	public void checkEMFValidationConstraints(be.ac.vub.simpleocl.resource.simpleocl.ISimpleoclTextResource resource, org.eclipse.emf.ecore.EObject root) {
		...
		if (new be.ac.vub.simpleocl.resource.simpleocl.util.SimpleoclRuntimeUtil().isEclipsePlatformRunning()) {
			// The EMF validation framework code throws a NPE if the validation plug-in is not
			...
			if (org.eclipse.emf.validation.internal.EMFModelValidationPlugin.getPlugin() != null) {
				try {
					...
					new be.ac.vub.simpleocl.resource.simpleocl.util.SimpleoclRuntimeUtil().logError("Exception while checking contraints provided by EMF validator classes.", t);
				}
			}
		}
	}
	
	public void addStatus(org.eclipse.core.runtime.IStatus status, be.ac.vub.simpleocl.resource.simpleocl.ISimpleoclTextResource resource, org.eclipse.emf.ecore.EObject root) {
		...
		if (!status.isMultiStatus() || !hasChildren) {
			if (status.getSeverity() == org.eclipse.core.runtime.IStatus.ERROR) {
				for (org.eclipse.emf.ecore.EObject cause : causes) {
					resource.addError(status.getMessage(), be.ac.vub.simpleocl.resource.simpleocl.SimpleoclEProblemType.ANALYSIS_PROBLEM, cause);
				}
			}
			if (status.getSeverity() == org.eclipse.core.runtime.IStatus.WARNING) {
				for (org.eclipse.emf.ecore.EObject cause : causes) {
					resource.addWarning(status.getMessage(), be.ac.vub.simpleocl.resource.simpleocl.SimpleoclEProblemType.ANALYSIS_PROBLEM, cause);
				}
			}
		}
		...
		if (uri != null && uri.isPlatform()) {
			...
			if (platformResource instanceof org.eclipse.core.resources.IFile) {
				...
				try {
					...
					new be.ac.vub.simpleocl.resource.simpleocl.util.SimpleoclRuntimeUtil().logWarning("Could not determine encoding of platform resource: " + uri.toString(), ce);
				}
			}
		}
		return null;
