 ...
 */
public class SimpleoclOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "org.eclipselabs.simpleocl.resource.simpleocl.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.eclipselabs.simpleocl.resource.simpleocl.ui.occurences.declaration";
	...
	private org.eclipselabs.simpleocl.resource.simpleocl.ui.SimpleoclTokenScanner tokenScanner;
	...
	private org.eclipselabs.simpleocl.resource.simpleocl.ISimpleoclTextResource textResource;
	 ...
	 */
	public SimpleoclOccurrence(org.eclipselabs.simpleocl.resource.simpleocl.ISimpleoclTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.eclipselabs.simpleocl.resource.simpleocl.ui.SimpleoclTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.eclipselabs.simpleocl.resource.simpleocl.mopp.SimpleoclMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(org.eclipselabs.simpleocl.resource.simpleocl.ui.SimpleoclBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, org.eclipselabs.simpleocl.resource.simpleocl.ui.SimpleoclPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, org.eclipselabs.simpleocl.resource.simpleocl.ui.SimpleoclPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, org.eclipselabs.simpleocl.resource.simpleocl.ui.SimpleoclPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == org.eclipselabs.simpleocl.resource.simpleocl.ui.SimpleoclPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(org.eclipselabs.simpleocl.resource.simpleocl.ui.SimpleoclOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.eclipselabs.simpleocl.resource.simpleocl.ui.SimpleoclOccurrence.DECLARATION_ANNOTATION_ID);
	}
