...
public abstract class AbstractSimpleoclDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			new org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclRuntimeUtil().logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			new org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclRuntimeUtil().logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(org.eclipselabs.simpleocl.resource.simpleocl.debug.ESimpleoclDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclDebugMessage message = new org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(org.eclipselabs.simpleocl.resource.simpleocl.debug.ESimpleoclDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(org.eclipselabs.simpleocl.resource.simpleocl.debug.ESimpleoclDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(org.eclipselabs.simpleocl.resource.simpleocl.debug.ESimpleoclDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(org.eclipselabs.simpleocl.resource.simpleocl.debug.ESimpleoclDebugMessageTypes.STARTED, true);
			setSuspend(true);
