...
public class SimpleoclDebugTarget extends org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclDebugElement implements org.eclipse.debug.core.model.IDebugTarget, org.eclipselabs.simpleocl.resource.simpleocl.debug.ISimpleoclDebugEventListener {
	
	/**
	 * associated system process
	 */
	private org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclDebugProcess process;
	...
	private org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclDebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<org.eclipselabs.simpleocl.resource.simpleocl.debug.ISimpleoclDebugEventListener> eventListeners = new java.util.ArrayList<org.eclipselabs.simpleocl.resource.simpleocl.debug.ISimpleoclDebugEventListener>();
	
	private org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclDebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new org.eclipselabs.simpleocl.resource.simpleocl.mopp.SimpleoclMetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclDebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclDebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((org.eclipselabs.simpleocl.resource.simpleocl.debug.ISimpleoclDebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public SimpleoclDebugTarget(org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclDebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclDebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclDebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(org.eclipselabs.simpleocl.resource.simpleocl.debug.ISimpleoclDebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(org.eclipselabs.simpleocl.resource.simpleocl.debug.ISimpleoclDebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new org.eclipselabs.simpleocl.resource.simpleocl.mopp.SimpleoclMetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclLineBreakpoint lineBreakpoint = (org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclLineBreakpoint) breakpoint;
	...
	public void handleMessage(org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclDebugMessage message) {
		try {
			if (message.hasType(org.eclipselabs.simpleocl.resource.simpleocl.debug.ESimpleoclDebugMessageTypes.STARTED)) {
				started();
			} else if (message.hasType(org.eclipselabs.simpleocl.resource.simpleocl.debug.ESimpleoclDebugMessageTypes.SUSPENDED)) {
				suspend();
			} else if (message.hasType(org.eclipselabs.simpleocl.resource.simpleocl.debug.ESimpleoclDebugMessageTypes.TERMINATED)) {
				terminated();
			} else if (message.hasType(org.eclipselabs.simpleocl.resource.simpleocl.debug.ESimpleoclDebugMessageTypes.RESUMED)) {
	...
	public org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclDebugProxy getDebugProxy() {
