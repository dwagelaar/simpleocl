...
public class SimpleoclDebugThread extends org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclDebugElement implements org.eclipse.debug.core.model.IThread, org.eclipselabs.simpleocl.resource.simpleocl.debug.ISimpleoclDebugEventListener {
	
	private boolean suspended = false;
	private org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclDebugTarget debugTarget;
	
	public SimpleoclDebugThread(org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public org.eclipselabs.simpleocl.resource.simpleocl.debug.SimpleoclDebugTarget getTarget() {
		return debugTarget;
	}
