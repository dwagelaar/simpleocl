 ...
 */
public class SimpleoclDebuggableInterpreter<ResultType, ContextType> extends org.eclipselabs.simpleocl.resource.simpleocl.debug.AbstractSimpleoclDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private org.eclipselabs.simpleocl.resource.simpleocl.util.AbstractSimpleoclInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private org.eclipselabs.simpleocl.resource.simpleocl.ISimpleoclCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public SimpleoclDebuggableInterpreter(org.eclipselabs.simpleocl.resource.simpleocl.util.AbstractSimpleoclInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new org.eclipselabs.simpleocl.resource.simpleocl.ISimpleoclInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public org.eclipselabs.simpleocl.resource.simpleocl.util.AbstractSimpleoclInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		org.eclipselabs.simpleocl.resource.simpleocl.ISimpleoclLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private org.eclipselabs.simpleocl.resource.simpleocl.ISimpleoclLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		org.eclipselabs.simpleocl.resource.simpleocl.ISimpleoclCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(org.eclipselabs.simpleocl.resource.simpleocl.debug.ESimpleoclDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclEObjectUtil.getDepth(current);
		stopCondition = new org.eclipselabs.simpleocl.resource.simpleocl.ISimpleoclCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclEObjectUtil.getDepth(current) - 1;
		stopCondition = new org.eclipselabs.simpleocl.resource.simpleocl.ISimpleoclCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
