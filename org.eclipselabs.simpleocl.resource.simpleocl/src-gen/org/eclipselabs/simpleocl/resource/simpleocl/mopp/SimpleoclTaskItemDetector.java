...
/**
 * The SimpleoclTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * SimpleoclTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class SimpleoclTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<org.eclipselabs.simpleocl.resource.simpleocl.mopp.SimpleoclTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<org.eclipselabs.simpleocl.resource.simpleocl.mopp.SimpleoclTaskItem> foundItems = new java.util.ArrayList<org.eclipselabs.simpleocl.resource.simpleocl.mopp.SimpleoclTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.eclipselabs.simpleocl.resource.simpleocl.mopp.SimpleoclTaskItem(keyword, message, localLine, offset, end));
