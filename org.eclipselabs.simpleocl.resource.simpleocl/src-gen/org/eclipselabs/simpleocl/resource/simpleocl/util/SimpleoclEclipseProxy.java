 ...
 */
public class SimpleoclEclipseProxy {
	...
	public void getDefaultLoadOptionProviderExtensions(java.util.Map<Object, Object> optionsMap) {
		if (org.eclipse.core.runtime.Platform.isRunning()) {
			...
			org.eclipse.core.runtime.IConfigurationElement configurationElements[] = extensionRegistry.getConfigurationElementsFor(org.eclipselabs.simpleocl.resource.simpleocl.mopp.SimpleoclPlugin.EP_DEFAULT_LOAD_OPTIONS_ID);
			for (org.eclipse.core.runtime.IConfigurationElement element : configurationElements) {
				try {
					org.eclipselabs.simpleocl.resource.simpleocl.ISimpleoclOptionProvider provider = (org.eclipselabs.simpleocl.resource.simpleocl.ISimpleoclOptionProvider) element.createExecutableExtension("class");
					final java.util.Map<?, ?> options = provider.getOptions();
					final java.util.Collection<?> keys = options.keySet();
					for (Object key : keys) {
						org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclMapUtil.putAndMergeKeys(optionsMap, key, options.get(key));
					}
				} catch (org.eclipse.core.runtime.CoreException ce) {
					new org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclRuntimeUtil().logError("Exception while getting default options.", ce);
				}
			}
		}
	}
	...
	public void getResourceFactoryExtensions(java.util.Map<String, org.eclipse.emf.ecore.resource.Resource.Factory> factories) {
		if (org.eclipse.core.runtime.Platform.isRunning()) {
			org.eclipse.core.runtime.IExtensionRegistry extensionRegistry = org.eclipse.core.runtime.Platform.getExtensionRegistry();
			org.eclipse.core.runtime.IConfigurationElement configurationElements[] = extensionRegistry.getConfigurationElementsFor(org.eclipselabs.simpleocl.resource.simpleocl.mopp.SimpleoclPlugin.EP_ADDITIONAL_EXTENSION_PARSER_ID);
			for (org.eclipse.core.runtime.IConfigurationElement element : configurationElements) {
				try {
	 ...
	 */
	public org.eclipselabs.simpleocl.resource.simpleocl.mopp.SimpleoclResource getResource(org.eclipse.core.resources.IFile file) {
	...
	public void checkEMFValidationConstraints(org.eclipselabs.simpleocl.resource.simpleocl.ISimpleoclTextResource resource, org.eclipse.emf.ecore.EObject root) {
		...
		if (new org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclRuntimeUtil().isEclipsePlatformRunning()) {
			// The EMF validation framework code throws a NPE if the validation plug-in is not
			...
			if (org.eclipse.emf.validation.internal.EMFModelValidationPlugin.getPlugin() != null) {
				try {
					...
					new org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclRuntimeUtil().logError("Exception while checking contraints provided by EMF validator classes.", t);
				}
			}
		}
	}
	
	public void addStatus(org.eclipse.core.runtime.IStatus status, org.eclipselabs.simpleocl.resource.simpleocl.ISimpleoclTextResource resource, org.eclipse.emf.ecore.EObject root) {
		...
		if (!status.isMultiStatus() || !hasChildren) {
			if (status.getSeverity() == org.eclipse.core.runtime.IStatus.ERROR) {
				for (org.eclipse.emf.ecore.EObject cause : causes) {
					resource.addError(status.getMessage(), org.eclipselabs.simpleocl.resource.simpleocl.SimpleoclEProblemType.ANALYSIS_PROBLEM, cause);
				}
			}
			if (status.getSeverity() == org.eclipse.core.runtime.IStatus.WARNING) {
				for (org.eclipse.emf.ecore.EObject cause : causes) {
					resource.addWarning(status.getMessage(), org.eclipselabs.simpleocl.resource.simpleocl.SimpleoclEProblemType.ANALYSIS_PROBLEM, cause);
				}
			}
		}
		...
		if (uri != null && uri.isPlatform()) {
			...
			if (platformResource instanceof org.eclipse.core.resources.IFile) {
				...
				try {
					...
					new org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclRuntimeUtil().logWarning("Could not determine encoding of platform resource: " + uri.toString(), ce);
				}
			}
		}
		return null;
