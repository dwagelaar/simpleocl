 ...
 */
public class SimpleoclMinimalModelHelper {
	
	private final static org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclEClassUtil eClassUtil = new org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = org.eclipselabs.simpleocl.resource.simpleocl.util.SimpleoclListUtil.castListUnchecked(value);
